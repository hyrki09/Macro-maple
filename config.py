...
# ===== 미니맵 좌표 인식 (PHASE 5) =====
# 미니맵에서 캐릭터 위치를 찾을 때 쓰는 캐릭터 점(char_dot) 템플릿 경로.
# 반드시 실제 게임 미니맵에서 캐릭터 점만 잘라 캡처해야 한다. (인터넷 이미지 X)
MINIMAP_TEMPLATE_PATH = 'assets/minimap/char_dot.png'

# 미니맵 캐릭터 점 템플릿 매칭 임계값 (0~1). 매칭 점수가 이 값 미만이면
# '캐릭터 점을 못 찾음(None)' 으로 본다. 점이 작아 오탐이 잦으면 올리고,
# 매칭이 잘 안 되면 0.6 까지 낮춰 max_val 로그를 보며 보정한다.
MINIMAP_MATCH_THRESHOLD = 0.70
